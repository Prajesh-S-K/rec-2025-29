...
// Problem Statement:
// Sam loves chocolates and starts buying them on the 1st day of the year. Each day of the
// year, x, is numbered from 1 to Y. On days when x is odd, Sam will buy x chocolates; on
// days when x is even, Sam will not purchase any chocolates.
...
// new line. This is a function-only challenge, so input is handled for you by the locked stub
// code in the editor.
// Input Format
// The program takes an array of integers as a parameter.
// The locked code in the editor handles reading the following input from stdin, assembling
// it into an array of integers (arr), and calling calculate(arr).
...
int main() {
	...
	int arr[T]; // array to store N values
